...
void DoorSimulation::initialize() {
 ...
 void DoorSimulation::update(double delta) {
    ...
    if (requestWasRead) {
       doorIsOpen = request.doorOpened;
       doorIsClose = request.doorClosed;
       isPerson = request.personSensor;
    }

    mState.open = this->targetDevice->getGpio("open");
    mState.close = this->targetDevice->getGpio("close");

    this->log() <<std::endl<< "Open door: " << mState.open <<"; Close door: "<< mState.close<<std::endl;

    this->log() << "Input:"<<std::endl<<" Opened: " << request.doorOpened << "; Closed: " << request.doorClosed << "; Persona: " << request.personSensor <<std::endl;
