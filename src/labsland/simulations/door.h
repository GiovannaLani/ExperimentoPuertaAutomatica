...
struct DoorRequest : public BaseInputDataType {
    ...
    bool deserialize(std::string const & input) {
        ...
        if (segments[0] == "1") doorOpened = true;
        else if (segments[0] == "0") doorOpened = false;
        else return false;

        if (segments[1] == "1") doorClosed = true;
        else if (segments[1] == "0") doorClosed = false;
        else return false;

        if (segments[2] == "1") personSensor = true;
        else if (segments[2] == "0") personSensor = false;
        else return false;
